...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Carrega o modelo pequeno de língua inglesa e o atribui a variável \textquotesingle{}nlp\textquotesingle{}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Atribui uma frase de exemplo a variável \textquotesingle{}text\textquotesingle{}}
...
\CommentTok{\# Guarda o resultado na variável \textquotesingle{}doc\textquotesingle{}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={O pipeline de PLN do spaCy: chamar nlp(texto) passa o texto por vários componentes (tokenizer, tagger, parser, lematizador, NER) e devolve um objeto Doc já anotado.}]{img/pipeline-spacy.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Percorre os itens no objeto Doc, usando a variável \textquotesingle{}token\textquotesingle{} para se referir aos itens da lista}
...
\ControlFlowTok{for}\NormalTok{ token }\KeywordTok{in}\NormalTok{ doc:}
    ...
    \CommentTok{\# Imprime o token e as anotações morfossintaticas genéricas e detalhadas}
...
\ControlFlowTok{for}\NormalTok{ token }\KeywordTok{in}\NormalTok{ doc:}
    ...
    \CommentTok{\# Imprime o token e o resultado da análise morfológica}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Recupera informação morfológica sobre aspecto para o Token no índice 22 do objeto Doc}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Recupera informação morfológica sobre aspecto para o Token no índice 21 do objeto Doc}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Recupera informação morfológica para o Token no índice 21 do objeto Doc}
\CommentTok{\# Usa o método to\_dict() para converter o resultado em um dicionário}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Atribui a informação morfológica ao dicionário \textquotesingle{}morph\_dict\textquotesingle{}}
...
\ControlFlowTok{for}\NormalTok{ token }\KeywordTok{in}\NormalTok{ doc:}
    ...
    \CommentTok{\# Imprime o token e sua etiqueta de dependência}
...
\ControlFlowTok{for}\NormalTok{ token }\KeywordTok{in}\NormalTok{ doc:}
    ...
    \CommentTok{\# Imprime o índice do token atual, o token, a dependência, o governante e seu índice}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Percorre as sentenças no objeto Doc e as conta usando enumerate()}
\ControlFlowTok{for}\NormalTok{ number, sent }\KeywordTok{in} \BuiltInTok{enumerate}\NormalTok{(doc.sents):}

    \CommentTok{\# Imprime o número e a sentença}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Imprime a entidade nomeada e os índices de seus tokens de início e fim}
...
\ControlFlowTok{for}\NormalTok{ token }\KeywordTok{in}\NormalTok{ doc[doc.ents[}\DecValTok{0}\NormalTok{].start: doc.ents[}\DecValTok{0}\NormalTok{].end]:}
    ...
    \CommentTok{\# Imprime o token e seu índice}
