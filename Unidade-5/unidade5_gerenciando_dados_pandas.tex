...
O pandas devolve o conteúdo do CSV em um \textbf{\texttt{DataFrame}}, sua estrutura de dados nativa. O método @head()@ mostra as primeiras linhas:
...
Cada coluna é, na verdade, um objeto \textbf{\texttt{Series}} --- pense no @DataFrame@ como a tabela inteira, cujas colunas são @Series@:
...
\subsection{Filtrando linhas com \textnormal{\texttt{.loc}}}
...
Mas alguns artigos \textbf{não} receberam comentários --- nesses casos, dividiríamos zero por zero. Para essas linhas, @comments_ratio@ fica marcada como \textbf{\texttt{NaN}} (\textit{not a number}). O pandas \textbf{ignora automaticamente} os @NaN@ nos cálculos, como mostra o @describe()@:
...
Ao criar uma coluna nova em @talk@, o pandas emite um \textbf{\texttt{SettingWithCopyWarning}}, porque @talk@ é apenas um recorte (\textit{slice}) do @DataFrame@ original. A solução é criar uma \textbf{cópia profunda} com @.copy()@:
...
Cada célula da coluna @processed_title@ agora contém um objeto \textbf{\texttt{Doc}} do spaCy:
...
\subsection{Definindo a própria função e aplicando com \textnormal{\texttt{apply()}}}
...
{\bfseries 1. Por convenção, o pandas é importado com qual apelido?}
\begin{enumerate}[leftmargin=1.6em,itemsep=0pt,topsep=2pt,label=\arabic*.]
  \item @pd@ \textcolor{tipc}{\textbf{(correta)}}
  \item @pandas@
  \item @dt@
\end{enumerate}

{\bfseries 2. Cada coluna de um DataFrame é, na verdade, um objeto:}
\begin{enumerate}[leftmargin=1.6em,itemsep=0pt,topsep=2pt,label=\arabic*.]
  \item @Series@ \textcolor{tipc}{\textbf{(correta)}}
  \item @DataFrame@
  \item @list@
\end{enumerate}

{\bfseries 3. Um valor NaN indica:}
\begin{enumerate}[leftmargin=1.6em,itemsep=0pt,topsep=2pt,label=\arabic*.]
  \item Um valor ausente (``não é um número'') \textcolor{tipc}{\textbf{(correta)}}
  \item O número zero
  \item Infinito
\end{enumerate}

{\bfseries 4. Qual operador combina dois critérios num filtro .loc?}
\begin{enumerate}[leftmargin=1.6em,itemsep=0pt,topsep=2pt,label=\arabic*.]
  \item @&@ \textcolor{tipc}{\textbf{(correta)}}
  \item @and@
  \item @+@
\end{enumerate}

{\bfseries 5. Por que criar uma cópia com .copy() de um recorte do DataFrame?}
\begin{enumerate}[leftmargin=1.6em,itemsep=0pt,topsep=2pt,label=\arabic*.]
  \item Para evitar o @SettingWithCopyWarning@ e não alterar o original \textcolor{tipc}{\textbf{(correta)}}
  \item Para acelerar o cálculo
  \item É obrigatório em toda operação
\end{enumerate}

{\bfseries 6. (Segurança) Por que \textit{não} usar read\_pickle em um arquivo de origem não confiável?}
\begin{enumerate}[leftmargin=1.6em,itemsep=0pt,topsep=2pt,label=\arabic*.]
  \item Porque pode executar código arbitrário na sua máquina \textcolor{tipc}{\textbf{(correta)}}
  \item Porque é mais lento que o CSV
  \item Porque não consegue ler texto
\end{enumerate}
